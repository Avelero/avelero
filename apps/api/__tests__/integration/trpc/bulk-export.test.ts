...
import { createTestBrand, testDb, cleanupTables } from "@v1/db/testing";
...
async function callExportStart(
    ...
    input: {
        selection: { mode: "all"; excludeIds?: string[] } | { mode: "explicit"; includeIds: string[] };
        filterState?: any;
        search?: string;
    },
) {
    ...
    const createCaller = createCallerFactory({ bulk: { export: exportRouter } } as any);
...
describe("Bulk Export TRPC Router", () => {
    ...
    describe("export.start", () => {
        it("creates export job and returns jobId", async () => {
            ...
            expect(mockTrigger).toHaveBeenCalledWith("export-products", expect.objectContaining({
                jobId: result.jobId,
                brandId,
                userEmail,
                selectionMode: "all",
    ...
    describe("export.status", () => {
        ...
        it("throws for non-existent job", async () => {
            ...
            await expect(
                callExportStatus(ctx, { jobId: "00000000-0000-0000-0000-000000000000" }),
        ...
        it("prevents cross-brand access", async () => {
            ...
            const jobId = await createTestExportJob(otherBrandId, otherUserId, otherEmail, {
                status: "COMPLETED",
            });
