-- =============================================
-- RESTORE: Database-level throttling for realtime broadcasts
--
-- Problem: The throttling logic from 20260113175235 was accidentally
-- removed by subsequent migrations that replaced the entire
-- broadcast_domain_changes() function without preserving the throttle logic.
--
-- This migration restores the throttling while keeping all recent fixes:
-- - DELETE/INSERT/UPDATE handling (from 20260114194142)
-- - FK brand_id resolution for child tables (from 20260114194142)
-- - SET search_path security fix (from 20260114174502)
-- - Notifications domain support (from 20260114191300)
--
-- Throttling ensures only one broadcast per domain/brand per second,
-- preventing message queue flooding during bulk operations (1000+ rows).
-- =============================================

-- Drop and recreate the function with complete fix + throttling
...
COMMENT ON FUNCTION realtime.broadcast_domain_changes() IS
  'Broadcasts domain-specific changes for realtime updates with 1-second throttling per domain/brand. Correctly handles DELETE operations and resolves brand_id through foreign keys for child tables. Uses advisory locks and broadcast_throttle table to prevent message flooding during bulk operations.';
