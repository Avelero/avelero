...
export function CarouselInput({ field }: CarouselInputProps) {
  ...
  const selectedCount =
    carouselConfig?.selectedCount ?? carouselConfig?.includeIds?.length ?? 0;
  ...
  const initialSelection = useMemo(
    ...
    [
      carouselConfig?.filter,
      carouselConfig?.includeIds,
      carouselConfig?.excludeIds,
    ],
  );
  ...
  const handleSave = useCallback(
    (selection: {
      ...
      const currentConfig = getConfigValue("carousel") as
