...
export function MenuInput({ menuType, configPath }: MenuInputProps) {
    const { getConfigValue, updateConfigValue, navigateToMenuItemEdit } = useDesignEditor();
    ...
    return (
        <div className="space-y-2">
            ...
            {viewState.type === "list" && items.length < 5 && <AddButtonRow onClick={handleAddClick} />}
            {viewState.type === "addUrl" && (
                <AddUrlInputRow onSubmit={handleAddUrlSubmit} onCancel={handleAddUrlCancel} />
            )}
