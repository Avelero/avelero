...
export function buildAttributeValueOptions(
    ...
    brandValues: Map<string, { id: string; name: string; taxonomyValueId: string | null }[]>,
    taxonomyValues: Map<string, { id: string; name: string; sortOrder: number; metadata: unknown }[]>,
): AttributeValueOption[] {
    const options: AttributeValueOption[] = [];

    // Get brand values for this attribute
    const brandVals = brandAttributeId ? brandValues.get(brandAttributeId) ?? [] : [];

    // Get taxonomy values for this attribute
    const taxVals = taxonomyAttributeId ? taxonomyValues.get(taxonomyAttributeId) ?? [] : [];
 ...
 * ```
 */
export function useAttributes({ brandAttributeId, taxonomyAttributeId }: UseAttributesParams) {
    ...
    }, [brandAttributeId, taxonomyAttributeId, brandAttributeValuesByAttribute, taxonomyValuesByAttribute]);
    ...
    const taxonomyValuesById = React.useMemo(() => {
        if (!taxonomyAttributeId) return new Map<string, { id: string; name: string; metadata: unknown }>();
