...
interface UseVariantFormOptions {
    ...
    initialData?: {
        ...
        ecoClaims: Array<{ ecoClaimId: string; claim: string }>;
        materials: Array<{ brandMaterialId: string; percentage: string | null }>;
        journey: Array<{ sortIndex: number; stepType: string; facilityId: string }>;
    };
...
export function useVariantForm(options: UseVariantFormOptions) {
    ...
    const buildInitialState = React.useCallback((): VariantFormState => {
        ...
        return {
            ...
            ecoClaims: initialData.ecoClaims.map((c) => ({ id: c.ecoClaimId, value: c.claim })),
    ...
    const { state, setField, updateField, setFields } = useFormState<VariantFormState>(
        buildInitialState()
    );
    ...
    const hasUnsavedChanges = React.useMemo(() => {
        ...
        // Compare eco claims
        if (state.ecoClaims.length !== orig.ecoClaims.length) return true;
        for (let i = 0; i < state.ecoClaims.length; i++) {
            if (state.ecoClaims[i]?.id !== orig.ecoClaims[i]?.id) return true;
        }
        ...
        for (let i = 0; i < state.journeySteps.length; i++) {
            ...
            if (curr?.facilityId !== origJ?.facilityId) return true;
