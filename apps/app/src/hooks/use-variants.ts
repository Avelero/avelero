...
type VariantsAction =
  ...
  | {
      type: "UPDATE_VARIANT";
      localId: string;
      updates: Partial<Omit<LocalVariant, "localId">>;
    }
  ...
  | {
      type: "REMOVE_VALUE_FROM_DIMENSION";
      attributeId: string;
      valueId: string;
    }
...
function variantsReducer(
  ...
  switch (action.type) {
        ...
        variants: state.variants.map((v) =>
          v.localId === action.localId
            ? {
                ...v,
                ...action.updates,
                status: v.upid ? "modified" : v.status,
              }
            : v,
        ),
      };
    ...
    case "ADD_DIMENSION": {
      ...
      if (
        state.dimensions.some(
          (d) => d.attributeId === action.dimension.attributeId,
        )
      ) {
        return state;
      }
    ...
    case "REMOVE_DIMENSION": {
      ...
      return {
        variants: updatedVariants.filter(
          (v) => v.upid || v.status !== "deleted",
        ),
    ...
    case "ADD_VALUE_TO_DIMENSION": {
      ...
      const dimIndex = state.dimensions.findIndex(
        (d) => d.attributeId === attributeId,
      );
      ...
      const activeVariants = updatedVariants.filter(
        (v) => v.status !== "deleted",
      );
      ...
      if (isFirstValueInDimension) {
        ...
        for (const otherCombo of otherCombinations) {
          ...
          const exists = state.variants.some((v) => {
            ...
            if (v.attributeValueIds.length !== newAttributeValueIds.length)
              return false;
...
export function useVariants(options?: UseVariantsOptions): UseVariantsReturn {
  ...
  const hydrateFromServer = React.useCallback(
    (serverVariants: ServerVariant[]) => {
      ...
      const dimensions: Dimension[] = Array.from(dimensionMap.values()).map(
        (d) => ({
