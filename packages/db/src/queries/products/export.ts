...
export async function getProductsForExport(
  ...
  const productRows = await db
    ...
    .leftJoin(
      brandManufacturers,
      eq(products.manufacturerId, brandManufacturers.id),
    )
    .leftJoin(brandSeasons, eq(products.seasonId, brandSeasons.id))
    .where(
      and(inArray(products.id, productIds), eq(products.brandId, brandId)),
    );
...
async function loadEnvironmentForProducts(
  ...
  Map<string, { carbonKg: number | null; waterLiters: number | null }>
> {
  const map = new Map<
    string,
    { carbonKg: number | null; waterLiters: number | null }
  >();
...
async function loadMaterialsForProducts(
  ...
  const rows = await db
    ...
    .leftJoin(
      brandMaterials,
      eq(productMaterials.brandMaterialId, brandMaterials.id),
    )
...
async function loadJourneyForProducts(
  ...
  const rows = await db
    .select({
      ...
      operatorName: brandOperators.displayName,
    })
    .from(productJourneySteps)
    .leftJoin(
      brandOperators,
      eq(productJourneySteps.operatorId, brandOperators.id),
    )
  ...
  for (const row of rows) {
    ...
    if (row.stepType && row.operatorName) {
      // Multiple operators per step are joined with semicolons
      if (journey[row.stepType]) {
        journey[row.stepType] += `; ${row.operatorName}`;
      } else {
        journey[row.stepType] = row.operatorName;
      }
    }
...
async function loadVariantsWithOverrides(
  ...
  for (const variant of variantRows) {
    ...
    variants.push({
      ...
      materialsOverride:
        materialsOverride && materialsOverride.length > 0
          ? materialsOverride
          : null,
      journeyStepsOverride:
        journeyOverride && Object.keys(journeyOverride).length > 0
          ? journeyOverride
          : null,
    });
...
async function loadAttributesForVariants(
  ...
  Map<string, Array<{ name: string; value: string; sortOrder: number }>>
> {
  const map = new Map<
    string,
...
async function loadMaterialsOverridesForVariants(
  ...
  const rows = await db
    ...
    .leftJoin(
      brandMaterials,
      eq(variantMaterials.brandMaterialId, brandMaterials.id),
    )
...
async function loadJourneyOverridesForVariants(
  ...
  const rows = await db
    ...
    .leftJoin(
      brandOperators,
      eq(variantJourneySteps.operatorId, brandOperators.id),
    )
