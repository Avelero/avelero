...
async function fetchVariantJourney(
  ...
  const rows = await db
    ...
    .innerJoin(
      brandOperators,
      eq(brandOperators.id, variantJourneySteps.operatorId),
    )
...
export async function variantHasOverrides(
  ...
  const [hasCommercial, hasEnvironment, hasMaterials, hasWeight, hasJourney] =
    await Promise.all([
      db
  ...
  return (
    hasCommercial || hasEnvironment || hasMaterials || hasWeight || hasJourney
  );
}
...
export async function getVariantOverridesOnlyById(
  ...
  const [coreData, variantEnvData, variantMaterialsData, variantJourneyData] =
    await Promise.all([
      fetchCoreVariantData(db, variantId),
      fetchVariantEnvironment(db, variantId),
      fetchVariantMaterials(db, variantId),
