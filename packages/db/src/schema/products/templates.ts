...
export const templateStatusEnum = ["draft", "published", "archived", "disabled"] as const;
...
export const templates = pgTable(
  ...
  (table) => ({
    ...
    brandTypeIdx: index("templates_brand_type_idx").on(table.brandId, table.templateType),
    brandStatusIdx: index("templates_brand_status_idx").on(table.brandId, table.templateStatus),
    brandEnabledIdx: index("templates_brand_enabled_idx").on(table.brandId, table.enabled),
    brandCreatedIdx: index("templates_brand_created_idx").on(table.brandId, table.createdAt),
    ...
    parentTemplateIdx: index("templates_parent_template_idx").on(table.parentTemplateId),
    ...
    // Validation indexes
    validationScoreIdx: index("templates_validation_score_idx").on(table.validationScore),

    // Language indexes
    primaryLanguageIdx: index("templates_primary_language_idx").on(table.primaryLanguage),
    ...
    templatesSelectForBrandMembers: pgPolicy("templates_select_for_brand_members", {
      as: "permissive",
      for: "select",
      to: ["authenticated"],
