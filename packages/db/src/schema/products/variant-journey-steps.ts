...
export const variantJourneySteps = pgTable(
  ...
  (table) => [
    // Unique constraint: allows multiple operators per step (same variant_id + sort_index)
    // but prevents duplicate operator assignments to the same step
    uniqueIndex("variant_journey_steps_variant_sort_operator_unq").on(
      table.variantId,
      table.sortIndex,
      table.operatorId,
    ),
    ...
    // Index for operator_id foreign key (optimizes JOINs and referential integrity checks)
    index("idx_variant_journey_steps_operator_id").using(
      "btree",
