 ...
 */
export const createTemplateSchema = templatesSchemas.dataSchema.extend({
 ...
 */
export const updateTemplateSchema = templatesSchemas.dataSchema.partial().extend({
 ...
 */
export const bulkUpdateTemplateSchema = z.object({
...
export const calculateTemplateCompleteness = (template: {
  ...
  if (template.validationRules && Object.keys(template.validationRules).length > 0) score += 1;
 ...
 */
export const validateTemplateConfig = (config: Record<string, any>): boolean => {
...
export const transformTemplateData = (data: any): any => {
  return {
    ...
    optionalFields: Array.isArray(data.optionalFields) ? data.optionalFields : [],
