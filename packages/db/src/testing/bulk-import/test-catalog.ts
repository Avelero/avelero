 ...
 *
 * Sets up brand catalog data (materials, seasons, operators, etc.)
...
export interface TestCatalogOptions {
  ...
  /** Custom operators to add (uses displayName) */
  operators?: string[];
  /** Custom tags to add */
  tags?: string[];
...
export interface InsertedCatalog {
  ...
  operators: Map<string, string>; // displayName -> id
...
export class TestCatalog {
  ...
  static async setup(
    ...
    if (options.operators?.length) {
      fixtures.operators = options.operators.map((displayName) => ({
        displayName,
        brandId,
      }));
    }
    ...
    // Insert operators (uses displayName)
    if (fixtures.operators.length > 0) {
      const inserted = await db
        .insert(schema.brandOperators)
        .values(fixtures.operators)
        .returning({
          id: schema.brandOperators.id,
          displayName: schema.brandOperators.displayName,
        });
      for (const row of inserted) {
        result.operators.set(row.displayName, row.id);
      }
    }
    ...
    if (options.attributes?.length) {
      ...
      attributeValuesToInsert = createAttributeValueFixtures(
        brandId,
        attributeIdMap,
      );
    }
