...
const defaultFieldConfigs: FieldConfig[] = [
  ...
  },
  {
    ...
    isEnabled: true,
    selectedSource: "featured_image",
  },
  {
    fieldKey: "product.webshopUrl",
    isEnabled: true,
    selectedSource: "online_store_url",
  },
  {
  ...
  storage: {
    from: (bucket: string) => ({
      ...
      createSignedUrl: async (path: string, _expiresIn: number) => ({
        data: {
          signedUrl: `https://mock.storage.example.com/${bucket}/${path}?signed=true`,
        },
        error: null,
      }),
    }),
 ...
 * ```
 */
export function createTestSyncContext(
  ...
  return {
    ...
    variantGlobalCap: options.variantGlobalCap ?? null,
    totalExistingVariants: options.totalExistingVariants ?? 0,
