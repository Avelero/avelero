...
export async function createTestProductForExport(
  ...
  if (options.journeySteps && Object.keys(options.journeySteps).length > 0) {
    let sortIndex = 0;
    for (const [stepType, operatorName] of Object.entries(
      ...
      const [operator] = await testDb
        ...
        .returning({ id: schema.brandOperators.id });
...
export async function createTestVariantWithOverrides(
  ...
  if (
    options.carbonKgOverride !== undefined ||
    options.waterLitersOverride !== undefined
  ) {
  ...
  if (
    options.journeyStepsOverride &&
    Object.keys(options.journeyStepsOverride).length > 0
  ) {
    let sortIndex = 0;
    for (const [stepType, operatorName] of Object.entries(
      options.journeyStepsOverride,
