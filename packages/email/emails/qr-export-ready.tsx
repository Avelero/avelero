...
interface QrExportReadyEmailProps {
  exportedVariants: number;
  downloadUrl: string;
  expiresAt: string;
}

export default function QrExportReadyEmail({
  exportedVariants,
  downloadUrl,
  expiresAt,
}: QrExportReadyEmailProps) {
  ...
  return (
    <Html>
      <Head />
      <Preview>Your QR code export is ready for download</Preview>
      <Tailwind>
        <Body className="bg-white my-auto mx-auto font-sans">
          <Container className="my-[40px] mx-auto max-w-[600px]">
            <Heading className="font-semibold text-center p-0 my-[24px] mx-0">
              Your QR export is ready
            </Heading>

            <Section className="mb-4 text-center">
              <Text className="text-neutral-600">
                Your QR export containing{" "}
                <strong>{exportedVariants.toLocaleString()} variants</strong> is
                ready for download.
              </Text>
            </Section>
...
export const previewProps: QrExportReadyEmailProps = {
  exportedVariants: 124,
