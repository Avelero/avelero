...
describe("LIFE-002: Second Integration is Secondary", () => {
  ...
  it("second connected integration has isPrimary = false", async () => {
    ...
    const secondaryId = await createTestBrandIntegration(
      brandId,
      "its-perfect",
      {
        isPrimary: false,
  ...
  it("only one integration can be primary at a time", async () => {
    ...
    // Connect secondary
    const secondary1Id = await createTestBrandIntegration(
      brandId,
      "its-perfect",
      {
        isPrimary: false,
    ...
    const secondary2Id = await createTestBrandIntegration(
      brandId,
      "elastic-suite",
      {
        isPrimary: false,
...
describe("LIFE-004: Disconnect Only Integration", () => {
      ...
      variants: [
        {
          sku: "SRC-001",
          barcode: "SRC-BC",
          externalVariantId: "gid://shopify/ProductVariant/222",
        },
      ],
    });
  ...
  it("brand can connect new integration after disconnecting", async () => {
    ...
    const newIntegrationId = await createTestBrandIntegration(
      brandId,
      "its-perfect",
      {
        isPrimary: true,
