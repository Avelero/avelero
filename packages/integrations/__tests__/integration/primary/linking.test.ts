...
describe("P-MATCH-001: Primary Does NOT Match Manual Products", () => {
    ...
    it("primary sync creates new product even when manual product has same barcode", async () => {
        ...
        const integrationProduct = allProducts.find((p) => p.source === "integration");
...
describe("P-ATTR-002: Primary Adds New Attribute Value", () => {
    ...
    it("adds new attribute value when external variant has new option value", async () => {
        ...
        expect(finalValues.map((v) => v.name).sort()).toEqual(["L", "M", "S", "XL"]);
    });
});
            ...
            options: [
                {
                    ...
                    linkedMetafield: null,
                    optionValues: [
                        { id: "1", name: "Adult", linkedMetafieldValue: null, swatch: null },
