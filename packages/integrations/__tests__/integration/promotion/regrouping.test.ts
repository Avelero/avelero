...
beforeAll(() => {
  registerConnector(
    getMockItsPerfectConnector() as Parameters<typeof registerConnector>[0],
  );
});
  ...
  it("updates isPrimary flags after promotion", async () => {
    ...
    setItsPerfectMockProducts([
      createItsPerfectMockProduct({
        ...
        variants: [createItsPerfectMockVariant({ barcode: "BC-001" })],
      }),
    ]);
...
describe("Promotion: Variant Re-Parenting", () => {
  ...
  it("moves variants to match new primary's grouping (3→1 merge)", async () => {
    ...
    const productIds = [
      ...new Set(allVariants.map((v) => v.product_variants.productId)),
    ];
...
describe("Promotion: Empty Product Archival", () => {
  ...
  it("archives products that become empty after re-grouping", async () => {
    ...
    // Check database state - products without variants should exist (archived/emptied)
    const allBrandProducts = await testDb
      .select()
      .from(products)
      .where(eq(products.brandId, brandId));

    // Get products with no variants (these are the "archived" ones)
    const emptyProducts: string[] = [];
    const activeProducts: typeof allBrandProducts = [];
    ...
    for (const product of allBrandProducts) {
      ...
      if (variants.length === 0) {
        emptyProducts.push(product.id);
      } else {
        activeProducts.push(product);
      }
    }

    // At least one product should have been emptied (archived)
    expect(emptyProducts.length).toBeGreaterThanOrEqual(1);

    // All non-empty products should have variants (already verified above)
