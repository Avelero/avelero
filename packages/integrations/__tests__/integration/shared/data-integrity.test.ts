...
describe("DATA-001: Variant UPID Never Changes", () => {
    ...
    it("each variant has a unique UPID", async () => {
        ...
        const mockProduct = createMockProduct({
            ...
            variants: [
                createMockVariant({ id: nextVariantId(), sku: "UNIQUE-1", barcode: "BC-1" }),
                createMockVariant({ id: nextVariantId(), sku: "UNIQUE-2", barcode: "BC-2" }),
                createMockVariant({ id: nextVariantId(), sku: "UNIQUE-3", barcode: "BC-3" }),
                createMockVariant({ id: nextVariantId(), sku: "UNIQUE-4", barcode: "BC-4" }),
            ],
        });
        setMockProducts([mockProduct]);
