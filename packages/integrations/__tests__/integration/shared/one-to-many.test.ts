...
describe("O2M-001: Detect One-to-Many Mapping", () => {
    ...
    it("detects one-to-many mapping (1 external product → multiple Avelero products)", async () => {
        ...
        const { product: productA, variants: variantsA } = await createExistingProduct({
            brandId,
            name: "Avelero Product A",
            variants: [{ sku: "PROD-A-001", barcode: "BARCODE-A1" }],
        });

        const { product: productB, variants: variantsB } = await createExistingProduct({
            brandId,
        ...
        const secondaryProduct = createMockProduct({
            ...
            description: "This product has variants from two different Avelero products",
            variants: [
                createMockVariant({
                    id: nextVariantId(),
                    sku: "PROD-A-001",
        ...
        const variantALink = variantLinks.find((l) => l.variantId === variantsA[0]!.id);
        const variantBLink = variantLinks.find((l) => l.variantId === variantsB[0]!.id);
...
describe("O2M-002: One-to-Many Duplicates Data to All Products", () => {
    ...
    it("product-level data is replicated to all matched Avelero products", async () => {
        ...
        const secondaryProduct = createMockProduct({
            ...
            description: "This description should propagate to matched products via overrides",
            variants: [
                createMockVariant({
                    id: nextVariantId(),
                    sku: "DUP-A-001",
...
describe("O2M-003: One-to-Many with Partial Variant Match", () => {
    ...
    it("primary handles partial match by creating new variants", async () => {
        ...
        const variantLinks = await testDb
            .select()
            .from(integrationVariantLinks)
            .where(eq(integrationVariantLinks.brandIntegrationId, primaryIntegrationId));
