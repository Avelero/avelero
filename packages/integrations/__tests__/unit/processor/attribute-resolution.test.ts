...
function createTestCaches(
    ...
    for (const val of values) {
        attrValues.set(`${val.attributeId}:${val.valueName.toLowerCase()}`, val.valueId);
    }
...
describe("resolveAttributeValueIds()", () => {
    test("single option resolves to ID", () => {
        ...
        const result = resolveAttributeValueIds([{ name: "Size", value: "M" }], caches);
    ...
    test("preserves order of matched options", () => {
        const caches = createTestCaches(
            ...
            [
                ...
                { attributeId: "attr-material", valueName: "Cotton", valueId: "uuid-material" },
            ]
        );
