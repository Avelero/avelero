  ...
  try {
    ...
    const phases: Array<{
    ...
    }> = [
      {
        phase: "preparing",
        phaseNumber: 0,
        execute: () => phase0_preparation(ctx),
      },
      {
        phase: "fetching",
        phaseNumber: 1,
        execute: () => phase1_fetchExternalStructure(ctx),
      },
      {
        phase: "computing",
        phaseNumber: 2,
        execute: () => phase2_computeAssignments(ctx),
      },
      {
        phase: "creating_products",
        phaseNumber: 4,
        execute: () => phase4_createMissingProducts(ctx),
      },
      {
        phase: "re_parenting",
        phaseNumber: 5,
        execute: () => phase5_reParentVariants(ctx),
      },
      {
        phase: "handling_orphans",
        phaseNumber: 6,
        execute: () => phase6_handleOrphanedVariants(ctx),
      },
      {
        phase: "archiving",
        phaseNumber: 7,
        execute: () => phase7_archiveEmptyProducts(ctx),
      },
      {
        phase: "updating_attributes",
        phaseNumber: 8,
        execute: () => phase8_handleAttributes(ctx),
      },
      {
        phase: "updating_links",
        phaseNumber: 9,
        execute: () => phase9_updateLinks(ctx),
      },
      {
        phase: "cleanup",
        phaseNumber: 11,
        execute: () => phase11_cleanup(ctx),
      },
    ];
...
async function phase0_preparation(ctx: PromotionContext): Promise<void> {
  ...
  const totalVariants = await getIntegrationVariantCount(
    db,
    ctx.config.brandId,
  );
 ...
 */
async function phase1_fetchExternalStructure(
  ctx: PromotionContext,
): Promise<void> {
...
async function phase2_computeAssignments(ctx: PromotionContext): Promise<void> {
  ...
  if (!externalGrouping) {
    throw new Error(
      "External grouping not available - phase 1 may have failed",
    );
  }
  ...
  (ctx as PromotionContextInternal).externalProductVariants =
 ...
 */
async function phase4_createMissingProducts(
  ctx: PromotionContext,
): Promise<void> {
  const db = ctx.db as Database;
  const externalGrouping = (ctx as PromotionContextInternal).externalGrouping;
  const externalProductVariants = (ctx as PromotionContextInternal)
    .externalProductVariants;

  if (!externalGrouping || !externalProductVariants) {
    throw new Error(
      "Required data not available - earlier phases may have failed",
    );
  }
  ...
  const variantAssignments = (ctx as PromotionContextInternal)
    .variantAssignments;
  ...
  const productGroups: Map<string, { productId: string; count: number }[]> =
    new Map();

  for (const [variantId, assignment] of variantAssignments) {
    if (assignment.targetExternalProductId) {
      const groups =
        productGroups.get(assignment.targetExternalProductId) ?? [];
      const existing = groups.find(
        (g) => g.productId === assignment.currentProductId,
      );
      if (existing) {
        existing.count++;
      } else {
  ...
  for (const [barcode, grouping] of externalGrouping) {
    const barcodes =
      externalProductsToBarcodes.get(grouping.externalProductId) ?? [];
...
async function phase5_reParentVariants(ctx: PromotionContext): Promise<void> {
  ...
  for (const [variantId, assignment] of variantAssignments) {
    if (assignment.targetExternalProductId) {
      const newProductId = mainProductMap.get(
        assignment.targetExternalProductId,
      );
 ...
 */
async function phase6_handleOrphanedVariants(
  ctx: PromotionContext,
): Promise<void> {
 ...
 */
async function phase7_archiveEmptyProducts(
  ctx: PromotionContext,
): Promise<void> {
  const db = ctx.db as Database;
...
async function phase8_handleAttributes(ctx: PromotionContext): Promise<void> {
  ...
  const existingAttributesMap = await loadBrandAttributesMap(
    db,
    ctx.config.brandId,
  );
  ...
  if (missingAttributeNames.length > 0) {
    ...
    for (const [name, id] of newAttributesMap) {
      existingAttributesMap.set(name.toLowerCase(), {
        id,
  ...
  const existingValuesMap = await loadAllBrandAttributeValuesMap(
    db,
    ctx.config.brandId,
  );
  ...
  for (const [attrName, values] of attributeValuesByName) {
    ...
    const existingValuesForAttr =
      existingValuesMap.get(attrId) ?? new Map<string, string>();
