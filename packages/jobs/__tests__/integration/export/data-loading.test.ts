...
describe("getProductsForExport()", () => {
    ...
    it("loads products with all related entities", async () => {
        ...
        expect(product.materials.some(m => m.name === "Cotton" && m.percentage === 60)).toBe(true);
        expect(product.materials.some(m => m.name === "Polyester" && m.percentage === 40)).toBe(true);
    ...
    it("filters by brandId (cannot load other brand products)", async () => {
        ...
        await createTestVariantWithOverrides(productId, brandId, { upid: "UPID-001" });
        ...
        await createTestVariantWithOverrides(otherProductId, otherBrandId, { upid: "UPID-002" });

        // Try to load both products with first brand's ID
        const products = await getProductsForExport(testDb, brandId, [productId, otherProductId]);
    ...
    it("loads manufacturer and season names via joins", async () => {
        ...
        await createTestVariantWithOverrides(productId, brandId, { upid: "UPID-MFR-001" });
    ...
    it("loads journey steps with facility names", async () => {
        ...
        await createTestVariantWithOverrides(productId, brandId, { upid: "UPID-JOURNEY-001" });
    ...
    it("handles products with no related data", async () => {
        ...
        await createTestVariantWithOverrides(productId, brandId, { upid: "UPID-MINIMAL-001" });
