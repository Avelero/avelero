...
describe("Variant Override Resolution", () => {
    ...
    it("variant with materials override includes materialsOverride array", async () => {
        ...
        expect(variant.materialsOverride!.some(m => m.name === "Recycled Polyester" && m.percentage === 70)).toBe(true);
        expect(variant.materialsOverride!.some(m => m.name === "Organic Cotton" && m.percentage === 30)).toBe(true);
    });
    ...
    it("variant with journey override includes journeyStepsOverride", async () => {
        ...
        expect(variant.journeyStepsOverride!["Raw Material"]).toBe("Alternative Farm");
        expect(variant.journeyStepsOverride!["Finishing"]).toBe("Special Finish Center");
    });
