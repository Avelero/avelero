...
describe("Cleanup Operations", () => {
    ...
    describe("Staging Data Cleanup", () => {
        it("cleans up staging data after successful commit", async () => {
            ...
            let count = await TestDatabase.getStagingProductCount(
                testDb,
                job.id
            );
            expect(count).toBe(5);
        ...
        it("cleanup isolates between jobs", async () => {
            ...
            job1Count = await TestDatabase.getStagingProductCount(
                testDb,
                job1.id
            );
            job2Count = await TestDatabase.getStagingProductCount(
                testDb,
                job2.id
            );
