...
async function autoCreateMissingEntities(
    ...
    if (missingManufacturers.length > 0) {
        const inserted = await database
            ...
            .returning({ id: schema.brandManufacturers.id, name: schema.brandManufacturers.name });
...
describe("CREATE_AND_ENRICH Mode - Entity Auto-Creation", () => {
    ...
    describe("Manufacturer Auto-Creation", () => {
        it("auto-creates manufacturer when not found in catalog", async () => {
            ...
            expect(parseResult.products[0]?.manufacturerName).toBe("New Textile Company");
            ...
            expect(updatedCatalog.manufacturers.has("new textile company")).toBe(true);
    ...
    describe("Facility Auto-Creation", () => {
        it("auto-creates facility when not found in catalog", async () => {
            ...
            expect(updatedCatalog.operators.has("modern textile factory india")).toBe(true);
            ...
            expect(dbFacilities.map((f) => f.displayName)).toContain("New Cotton Farm Brazil");
        });
    });
    ...
    describe("Attribute Value Auto-Creation", () => {
        it("auto-creates attribute value when not found in catalog", async () => {
            ...
            expect(updatedCatalog.attributeValues.has(`${styleAttrId}:formal`)).toBe(true);

            // Verify in database
            const dbAttributeValues = await testDb.query.brandAttributeValues.findMany({
    ...
    describe("Reuse Existing Entities", () => {
        it("reuses existing entity if found instead of creating duplicate", async () => {
            ...
            expect(updatedCatalog.manufacturers.get("existing manufacturer")).toBe(existingId);
    ...
    describe("Category Exception", () => {
        it("does NOT auto-create category - categories must exist", async () => {
            ...
            expect(updatedCatalog.categories.has("unknown category path")).toBe(false);
