...
describe("Journey Processing", () => {
  ...
  describe("Individual Journey Steps", () => {
    it("links Raw Material facility", async () => {
      ...
      expect(parseResult.products[0]?.journeySteps["Raw Material"]).toBe(
        "Cotton Farm Italy",
      );
    ...
    it("links Weaving facility", async () => {
      ...
      expect(parseResult.products[0]?.journeySteps.Weaving).toBe(
        "Textile Mill Portugal",
      );
    ...
    it("links Dyeing/Printing facility", async () => {
      ...
      expect(parseResult.products[0]?.journeySteps["Dyeing / Printing"]).toBe(
        "Dyeing Factory Spain",
      );
    ...
    it("links Stitching facility", async () => {
      ...
      expect(parseResult.products[0]?.journeySteps.Stitching).toBe(
        "Stitching Workshop Poland",
      );

      const brandCatalog = await loadBrandCatalog(testDb, brandId);
      expect(
        brandCatalog.operators.get("stitching workshop poland"),
      ).toBeDefined();
    });
    ...
    it("links Assembly facility", async () => {
      ...
      expect(parseResult.products[0]?.journeySteps.Assembly).toBe(
        "Assembly Plant Germany",
      );

      const brandCatalog = await loadBrandCatalog(testDb, brandId);
      expect(
        brandCatalog.operators.get("assembly plant germany"),
      ).toBeDefined();
    });
    ...
    it("links Finishing facility", async () => {
      ...
      expect(parseResult.products[0]?.journeySteps.Finishing).toBe(
        "Finishing Center Netherlands",
      );

      const brandCatalog = await loadBrandCatalog(testDb, brandId);
      expect(
        brandCatalog.operators.get("finishing center netherlands"),
      ).toBeDefined();
    });
  });
