...
describe("Excel Export - Correction Generation", () => {
  ...
  describe("Error Highlighting", () => {
    ...
    it("handles rows with multiple errors", async () => {
      const rows: ExportRow[] = [
        createExportRow(2, { "Product Title": "", SKU: "", Category: "" }, [
    ...
    it("handles rows with no errors", async () => {
      ...
      expect(getCellFillColor(worksheet.getCell(2, 1))).not.toBe(
        ERROR_FILL_COLOR,
      );
      expect(getCellFillColor(worksheet.getCell(2, 2))).not.toBe(
        ERROR_FILL_COLOR,
      );
      expect(getCellFillColor(worksheet.getCell(2, 3))).not.toBe(
        ERROR_FILL_COLOR,
      );
    });
  });
  ...
  describe("Column Auto-Sizing", () => {
    ...
    it("respects maximum column width", async () => {
      ...
      const rows: ExportRow[] = [createExportRow(2, { Column: veryLongValue })];
  ...
  describe("Full Correction Export", () => {
    it("generates correction export with all rows", async () => {
      ...
      expect(getCellFillColor(worksheet.getCell(4, 2))).not.toBe(
        ERROR_FILL_COLOR,
      ); // SKU-003
    });
  });
  ...
  describe("Error-Only Correction Export", () => {
    ...
    it("preserves original data in error rows", async () => {
      const allRows = [
        ...
        {
          rowNumber: 3,
          data: { Title: "Invalid Product", SKU: "", Category: "Electronics" },
        },
      ];
