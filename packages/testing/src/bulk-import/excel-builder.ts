...
function buildRowData(
  ...
  if (isParentRow) {
    ...
    if (variant.environmentalOverride) {
      ...
      row["Liters Water Used"] = variant.environmentalOverride.litersWaterUsed;
      row["Carbon Footprint"] = variant.environmentalOverride.carbonFootprint;
    ...
    if (variant.materialsOverride && variant.materialsOverride.length > 0) {
      const { names, percentages } = formatMaterials(variant.materialsOverride);
      row["Materials"] = names;
      row["Percentages"] = percentages;
    }
